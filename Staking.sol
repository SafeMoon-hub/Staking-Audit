...
contract StakingContract is Ownable, ReentrancyGuard{
    ...
    uint256 public withdrawFee = 1000; // emergency
    uint256 public ref = 0;
    uint256 public period = 15 days;
    uint256 public lockPeriod = 15 days;
    ...
    uint256 public depositsPending;  
    ...
    constructor(address _feeWallet, address _tokenContract) {
        require(_feeWallet!=address(0),"Please provide a valid dev wallet address");
        require(_tokenContract!=address(0),"Please provide a valid token contract address");
       
        feeReceiver = _feeWallet;
        TokenContract = _tokenContract;
    }

    function setFeeWallet(address _feeWallet) public onlyOwner {
        require(_feeWallet != address(0),"Please provide a valid address");
        feeReceiver = _feeWallet;
    }
    ...
    function deposit(uint256 _amount, address _referrer) external nonReentrant{
        ...
        require(_amount > 0, "you can deposit more than 1 token");
    ...
    function claimAllReward() public nonReentrant {
        ...
        require(block.timestamp > rewardsClaimed[msg.sender] + 24 hours , "Can only claim once every 24 hours");

        rewardsClaimed[msg.sender] = block.timestamp;
        ...
        require(balance >= depositsPending, "inconsistent pool state");

        uint256 availableRewards = balance.sub(depositsPending);
        require(availableRewards >= amountToSend, "Not enough reward tokens in pool");
    ...
    function withdraw(uint256 id) public nonReentrant {
        ...
        uint256 userAmount = depositState[id].depositAmount;
        depositsPending -= userAmount; 
        uint256 withdrawAmount;

        withdrawAmount = userAmount;

        uint256 amountToSend = withdrawAmount ;
    ...
    function emergencyWithdraw(uint256 id) public nonReentrant {
        require(
            depositState[id].investor == msg.sender,
            "only investor of this id can withraw"
        );
        ...
        if(withdrawFee > 0){
            uint256 fee = (userAmount * withdrawFee).div(percentRate);
            withdrawAmount = userAmount.sub(fee);
        } else {
            withdrawAmount = userAmount;
        }
    ...
    function changePoolStatus(bool _status) external onlyOwner{
        isPoolOpen = _status;
    }
